...
var methods = require('norel').methods;
...
function KnexBuilder(model) {
    ...
    //TODO too many times called
    _(methods).without('update', 'where').forEach(function(method) {
        //attach method to instance too
...
KnexBuilder.prototype.create = function(params) {
    ...
    return Promise.resolve(params).then(function(data) {
        ...
        return Promise.map(callbackQueue, function(callback) {
...
//TODO capitalize on this for model return
KnexBuilder.prototype.then = function(resolve, reject, progress) {
    ...
    return Promise.resolve().then(function() {
        ...
        var callbackQueue = [];
        if (self._method === 'update') {
            callbackQueue = model._eventQueue.afterUpdate;
        }
        Promise.map(callbackQueue, function(callback) {
            return callback(data);
        });
        return data;
